...
std::shared_ptr<draco::Uint32Array> ReaderMG2::readGridIndices(std::shared_ptr<ICTMRStream> stream, std::shared_ptr<draco::Float32Array> vertices)
{
    ...
    auto gridIndices = std::make_shared<draco::Uint32Array>(size);
...
void ReaderMG2::readIndices(std::shared_ptr<ICTMRStream> stream, std::shared_ptr<draco::Int32Array> indices)
{
...
void ReaderMG2::readVertices(std::shared_ptr<ICTMRStream> stream, std::shared_ptr<draco::Float32Array> vertices)
{
...
void ReaderMG2::readNormals(std::shared_ptr<ICTMRStream> stream, std::shared_ptr<CTMFileBody> body)
{
    ...
    LZMA::decompress(stream, stream, interleaved, interleaved->data->getLength());
    auto& ivs    = body->indices;
    auto  u32    = std::make_shared<draco::Uint32Array>(ivs->getBuffer(), ivs->getByteOffset(), ivs->getLength());
