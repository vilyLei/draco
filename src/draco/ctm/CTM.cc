...
std::shared_ptr<InterleavedStream> CTM::InterleavedStream(std::shared_ptr<draco::DataView> data, Int32 count)
{
    return std::make_shared<draco::InterleavedStream>(data, count);
}

void CTM::restoreIndices(std::shared_ptr<draco::Int32Array> pindices, Int32 len)
{
...
std::shared_ptr<draco::Float32Array> CTM::calcSmoothNormals(std::shared_ptr<draco::Uint32Array> pindices, std::shared_ptr<draco::Float32Array> pvertices)
{
    auto  psmooth = std::make_shared<draco::Float32Array>(pvertices->getLength());
    auto& smooth  = (*psmooth);
    Int32 indx,
        indy,
        indz,
...
void CTM::restoreVertices(std::shared_ptr<draco::Float32Array> pvertices, std::shared_ptr<FileMG2Header> pgrid, std::shared_ptr<draco::Uint32Array> pgridIndices, Float32 precision)
{
    auto& vertices    = (*pvertices);
    auto& grid        = (*pgrid);
    ...
    auto  pintVertices = std::make_shared<draco::Uint32Array>(vertices.getBuffer(), vertices.getByteOffset(), vertices.getLength());
...
void CTM::restoreGridIndices(std::shared_ptr<draco::Uint32Array> pgridIndices, Int32 len)
{
    auto& gridIndices = (*pgridIndices);
    auto  i           = 1;
    for (; i < len; ++i)
...
void CTM::restoreNormals(std::shared_ptr<draco::Float32Array> pnormals,
                         std::shared_ptr<draco::Float32Array> psmooth,
                         Float32                                    precision)
{
    ...
    auto  pintNormals = std::make_shared<draco::Uint32Array>(normals.getBuffer(), normals.getByteOffset(), normals.getLength());
...
void CTM::restoreMap(std::shared_ptr<draco::DataView> pmap, Int32 count, Float32 precision)
{
    auto& map = (*pmap);
    Int32 delta,
        value,
    ...
    auto  pintMap = std::make_shared<draco::Uint32Array>(map.getBuffer(), map.getByteOffset(), map.getLength());
    auto& intMap  = (*pintMap);
    auto  pf32Map = std::make_shared<draco::Float32Array>(map.getBuffer(), map.getByteOffset(), map.getLength());
    auto& f32Map  = (*pf32Map);
    for (; i < count; ++i)
    {
        delta = 0;
