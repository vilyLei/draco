...
InterleavedStream::InterleavedStream(std::shared_ptr<draco::DataView> pdata, Int32 pcount) noexcept
{
    //std::cout << "      InterleavedStream::InterleavedStream(), ###$$$ offset: " << pdata->getByteOffset() << ", byteLength: " << pdata->getByteLength() << std::endl;
    data = std::make_shared<draco::Uint8Array>(pdata->getBuffer(), pdata->getByteOffset(), pdata->getByteLength());
